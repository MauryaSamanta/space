...
def generate_scenario_v2(n_satellites=3, seed=42):

    np.random.seed(seed)

    state = {}

    base_i = 0.1  # FIXED near-equatorial
    base_raan = 0 # FIXED RAAN  
    base_a = 7250
    M_base = np.random.uniform(0, 2*np.pi)

    for i in range(n_satellites):

        # 🔥 FORCE ONE TRUE COLLISION
        if i == 0:
            r_t, v_t = sample_orbit(base_i, base_raan, M_base)

            # very small separation
            r_d = r_t + np.array([0.05, 0, 0])

            # SAME velocity → guaranteed intersection
            v_d = v_t.copy()
